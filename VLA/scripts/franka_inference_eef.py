...
# from residual_controller.bridge_controller_no_visual import load_bridge_controller
...
from residual_controller.controller_dataset import normalize_actions, denormalize_actions
...
class RosOperator:
    ...
    def arm_publish(self, right):
        ...
        target_gripper_pos = int(np.clip(gripper_pos_raw, 30, 220))  # Position (255 is fully closed)
        current_gripper_state = self.gripper_state_deque[-1]

        # Parameters
        deadband = 6  # No movement within this range of target

        # Apply deadband to prevent tiny movements
        if self.last_gripper_cmd is None:
            self.last_gripper_cmd = target_gripper_pos
        elif abs(target_gripper_pos - self.last_gripper_cmd) < deadband:
            gripper_pos = self.last_gripper_cmd
        else:
            ...
            print("Update gripper position:", gripper_pos)
...
def model_inference(args, config, ros_operator):
    ...
    def goto_homepose():
        ...
        # right1 = np.array([0.33453325,  0.45457331,  0.47811615, -0.77818588,  0.49373628, -0.32791843, -0.20765532,40])# watercup pick
        
        right1 = np.array([0.49840733, -0.0445624,   0.48697153,  0.99948938,  0.01178482,  0.02939177, 0.00426953,100]) # wipe pick

        # right1 = np.array([ 0.52404313,  0.29712474,  0.45152889, -0.64328936,  0.69178505, -0.21392064, -0.248697, 50  ]) # mango pick
        # right1 = np.array([ 4.87415257e-01,  1.33694195e-01,  5.55746489e-01, -9.63823436e-01, 5.90333134e-02,  8.12245637e-04, -2.59920742e-01,206]) # mango peel
    ...
    if args.use_controller:
        if args.use_controller =='bridge':
            controller = load_bridge_controller(use_force= True)
        elif args.use_controller =='lstm':
            controller = load_lstm_controller()
        ...
        while not rospy.is_shutdown() and not should_exit:
            ...
            while t < max_publish_step and not rospy.is_shutdown() and not should_exit:
                ...
                if paused and not should_exit:
                    ...
                    instruct_index = int(input("Enter the number of your choice: "))
                    if instruct_index>= len(all_instructions_list):
                        goto_homepose()
                        instruct_index = int(input("Enter the number of your choice: "))
                    lang_embeddings = lang_dict[lang_dict['all_instructions'][instruct_index]]
                ...
                act_chunk_execute_step = 48
                refine_act_execute_step = 48

                print(f"before VLA upadte t value:{t}")
                if t % act_chunk_execute_step == 0:
                    print("################### Inferenece new VLA action chunk #######################")
                    s1 = time.time()
                    vla_action_buffer, vla_tensor = inference_fn(config, policy)
                    e1 = time.time()
                    # print("VLA inference speed:", e1 - s1)
                ...
                if args.use_controller:
                    ...
                    # mask = vla_tensor[:, :, -1] > 1.0
                    # if mask.sum() != mask.numel():
                    #     print("Small value detected!!!!!!!!!!!!!!!!!!!")
                    # print(mask.sum())
                    # vla_tensor[:, :, -1][mask] /= 255

                    vla_tensor[:, :, -1] /= 255
                    # print("vla_tensor:", vla_tensor[:, 0:10, -1])
                    # print(f"vla_tensor: {vla_tensor[:,:,-1].max()}, {vla_tensor[:,:,-1].min()}")
                    if args.use_controller == 'bridge':
                        with torch.no_grad():

                            refined_trajectory = controller.predict(current_state,
                                                                    vla_tensor[:,:act_chunk_execute_step,:],
                                                                    img_cam1,
                                                                    img_cam2,
                                                                    gelsight)
                            refined_actions = refined_trajectory.squeeze(0).cpu().numpy()
                            e2 = time.time()
                            # print("Bridge inferece time :", e2 - e1)

                            # print(f"refined_bridge_actions: {refined_actions[0:10,-1]}")
                            refined_actions[:,-1] *=255
                            
                            print("Execute Bridge action")
                            for k in range(0, refine_act_execute_step):
                                # Check again if paused or should exit
                                if paused or should_exit:
                                    break

                                refined_act = refined_actions[t%refine_act_execute_step]
                        ...
                        with torch.no_grad():
                            ...
                            for k in range(0, act_chunk_execute_step):
                                ...
                                # print("refined_action_np:", refined_action_np.shape)
                                # print("refined_action_np:", refined_action_np[-1])
                    ...
                    # Check if paused
                    vla_act = vla_action_buffer[t%act_chunk_execute_step]
                    if not paused:
...
def get_arguments():
    ...
    # parser.add_argument('--arm_right_topic', action='store', type=str, help='arm_right_topic',
    #                     default='/equilibrium_pose', required=False)
...
def main():
    ...
    task_name = "wipe"  # wipe, mango, watercup
    task_path = f"./checkpoints/rdt-finetune-1b-{task_name}-rgb"
    ckpt_path = os.path.join(task_path, "checkpoint-40000")

    args.use_controller = 'bridge'  # 'lstm', 'bridge' None
    if args.use_controller:
