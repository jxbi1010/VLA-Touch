...
def run_producer(seed, num_workers, worker_id, fill_up, clean_dirty, dataset_type):
    ...
    for episode_steps in vla_dataset:
        ...
        if files_processed >= MAX_FILES_TO_PROCESS:
            print(f"Worker {worker_id}: Reached file limit of {MAX_FILES_TO_PROCESS}. Stopping.")
            break
        ...
        # Increment file counter after processing each episode
        files_processed += 1
        if files_processed % 10 == 0:
            print(f"Worker {worker_id}: Processed {files_processed}/{MAX_FILES_TO_PROCESS} files")
...
if __name__ == '__main__':
    ...
    # Add new argument for file limit
    ...
    # Update the global constant with command line argument
    MAX_FILES_TO_PROCESS = args.max_files
    print(f"Processing maximum of {MAX_FILES_TO_PROCESS} files")
