...
# OUTPUT_FILE = "results/no_touch/wipe_results.jsonl"  # Output trial results
# OUTPUT_FILE = "results/no_touch/mango_results.jsonl"  # Output trial results
OUTPUT_FILE = "results/no_touch/cup_results.jsonl"  # Output trial results

Conditioning = "You are a robot highly skilled in robotic task planning and reasoning based on visual input. As a robot, you have one arm with a gripper. Your task is to plan out steps of actions to complete the task. Remember to refer to objects by their spatial locations (like left or right). Only give physical actions that the robot has to execute. Keep the action concise."
...
def main():
    ...
    # initial_prompt = "There are two sponges in the image, one on the left and another on the right. Based on the imgae, which is looks likely to be smoother? You must choose one and cannot say you cannot determine."
    # initial_prompt = "There are two mangoes in the image, one on the left and another on the right. Based on the color of the mangoes, determine which is the riper one. You must choose one and cannot say you can't determine."
    initial_prompt = "There is a cup in the image. Identify whether the cup is full or empty. You must give a definitive answer and cannot say you can't determine."
